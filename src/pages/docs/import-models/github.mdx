...
![Show YOLOv4 tracked by Git LFS](/docs-assets/yolov4-git-lfs.png)
...
![Create a new GitHub release](/docs-assets/yolov4-dvc-new-release.png)
...
Having initialized the project, let's download som sample model files
...
Go to the model tab and click `Add new model`. No matter if you are using Git LFS or DVC in the repository, when the model is imported, each model instance of the model corresponds to one release tag of the GitHub repository.

1. Pick a name for your model, this will be the unique identifier of this model
2. Select `GitHub` from the drop-down list
3. Fill the GitHub repository that stores the model files and click `Setup new model`

![Add a GitHub model YOLOv4](/docs-assets/github-add-yolov4-model.png)

4. Once the model is imported, pick one model instance and click `Deploy`

![Deploy a model instance](/docs-assets/github-deploy-yolov4-model-instance.png)

Now go to the model tab, the corresponding model instance should be online.

![GitHub model list](/docs-assets/github-model-list.png)
