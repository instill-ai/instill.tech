...
You can follow the tutorial via the `Pipeline` tab or navigate to the `Data source` tab.

![Pipeline list page of the VDP Console](/docs-assets/start-here/pipeline-list-empty.png)
...
![Add a HTTP Source to create a pipeline in the VDP Console](/docs-assets/start-here/add-a-source-http.png)

> Check our growing list of [Source connectors](/docs/connectors/overview).
...
![Import a model from a GitHub repo via VDP Console](/docs-assets/start-here/add-a-model.png)
...
![Deploy a model instance via VDP Console](/docs-assets/start-here/deploy-a-model-instance.png)
...
![Add a HTTP Destination to create a pipeline in the VDP console](/docs-assets/start-here/add-a-destination-http.png)

Almost done! Just give your pipeline a unique id and an optional description, and click `Set up pipeline`.

![Set up a pipeline in the VDP Console](/docs-assets/start-here/set-up-a-pipeline.png)
...
http://localhost:8082/v1alpha/source-connectors
...
curl -X POST \
http://localhost:8083/v1alpha/models/mobilenetv2/instances/v1.0-cpu:deploy
...
http://localhost:8082/v1alpha/destination-connectors
...
http://localhost:8081/v1alpha/pipelines
...
**🙌 That's it!** You just built your first `SYNC` pipeline and triggered it to convert unstructured image data into structured, analysable insight.
