...
:::info
If your models are managed by Git, please check [GitHub](github). VDP supports importing models managed by Git LFS or DVC from a GitHub repository.
:::
...
Go to the model tab in the console and click `Add new model`. The setup guide assumes your model is stored in a GCS bucket. When the model is successfully imported, each model instance of the model corresponds to one ArtiVC tag.

1. Pick a name for your model, this will be the unique identifier of this model
2. Select `ArtiVC` from the drop-down list
3. Fill the following information and click `Setup new model`
   - Fill the GCS bucket path in which the model is stored, e.g., `gs://mybucket/path/to/mymodel/`
   - If the model is stored in a public GCS bucket, skip this step. Otherwise, get your application default credential or service account credential in JSON format and dump it into the `Credentials JSON` field. See [here](#find-the-credentials-json-file) to find the credentials json file.

![Add a ArtiVC model YOLOv4](/docs-assets/artivc-add-yolov4-model.png)

4. Once the model is imported, pick one model instance and click `Deploy`

![Deploy a model instance](/docs-assets/artivc-deploy-yolov4-model-instance.png)

Now go to the model tab, the corresponding model instance should be online.

![ArtiVC model list](/docs-assets/artivc-model-list.png)
...
```bash cURL
curl -X POST 'http://localhost:8083/v1alpha/models' \
...
```bash remote
curl -X POST 'http://localhost:8083/v1alpha/models/yolov4/instances/v1.0-cpu:test' \
--header 'Content-Type: application/json' \
--data-raw '{
    "inputs": [
        {
