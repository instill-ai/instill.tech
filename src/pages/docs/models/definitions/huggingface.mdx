...
```bash cURL
curl --location --request POST 'http://localhost:8083/v1alpha/models' \
...
```bash remote
curl -X POST 'http://localhost:8083/v1alpha/models/vit-base-patch16-224/instances/latest:test' \
--header 'Content-Type: application/json' \
--data-raw '{
    "inputs": [
        {
