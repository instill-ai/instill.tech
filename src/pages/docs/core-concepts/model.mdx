...
🎯 **VDP captures the versioning of your models.** During model development, ML teams version models to track and manage changes and collaborate between members.
...
To import your models into VDP, please refer to the [Prepare models](../models/preparation/overview) guideline.
...
That's why we integrate VDP with various platforms and tools as model sources and strive to support as many popular model sources as possible.

The model source in VDP is defined as `model definition`. A `model definition` defines the schema about how to configure and import the model. Currently, VDP supports importing models from the following sources:
...
🎯 **VDP provides automatic model inference server.**
...
curl -X POST 'http://localhost:8083/v1alpha/models/{model-id}/instances/{instance-id}:test' \
...
### Use model in pipeline for inference
...
The examples below showcase pipeline recipes that uses single or multiple model instances.
