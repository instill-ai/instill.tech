...
## 4. **Configure Output in Response Component**
    - **General Output**:
        - To extract content scraped from the webpage by Website Component, add **'Webpage Text Content'** field with Title **`Webpage Content`**, Key **`webpage_content`** and Value **`${web_scraper.output.pages[0].link_text}`**.
    - **Specific Outputs:**
