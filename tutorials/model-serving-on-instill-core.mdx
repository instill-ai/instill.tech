...
with **🔮 Instill Core**, and on the cloud with **☁️ Instill Cloud**.
...
2. No Code Console Builder: Easily utilize custom models defined with **⚗️
   Instill Model** as modular AI components via **📺 Instill Console**, allowing
...
own custom model with **Instill Model** for local deployment with **🔮 Instill
Core**.
...
2. **⌨️ Instill CLI**: The easiest way to launch **🔮 Instill Core** for local
   deployment is via the **⌨️ Instill CLI**. To install **⌨️ Instill CLI** using
   Homebrew, please run the following command in your terminal:
   ```bash
   brew install instill-ai/tap/inst
   ```
3. **Launch 🔮 Instill Core**: To launch simply run:
   ```bash
   inst local deploy
   ```
...
<ZoomableImg src="/tutorial-assets/model-serving-on-instill-core/T1.png" alt="Configure Model Settings" />
...
[TinyLlama-1.1B-Chat](https://huggingface.co/TinyLlama/TinyLlama-1.1B-Chat-v1.0)
model falls under the [Text Generation Chat](https://www.instill.tech/docs/model/ai-task#text-generation-chat) AI task, and so we will make use of
...
define the input format for the [Text Generation Chat](https://www.instill.tech/docs/model/ai-task#text-generation-chat) AI task, and the
...
deploy this model locally with **🔮 Instill Core**!
...
able to login to the hosted Docker registry. To do this, you first need to create an API token by:
...
<ZoomableImg src="/tutorial-assets/model-serving-on-instill-core/T2.png" alt="Create API Token" />

Now we can login to the Docker register in the **🔮 Instill Core** instance by running:
```bash
...
on building **💧 Instill VDP** pipelines with **📺 Instill Console**.
