...
**⚗️ Instill Model** is a sophisticated MLOps/LLMOps platform specially designed
to orchestrate model serving, fine-tuning, and monitoring to ensure consistent
and reliable performance. Offering robust model orchestration capabilities, it
allows for efficient management and deployment of deep learning models for
unstructured data ETL. It is used to deploy and serve models locally on **🔮
Instill Core**, and on **☁️ Instill Cloud**.

## Why use ⚗️ Instill Model?

1. Seamless Integration with **💧 Instill VDP**: integrate effortlessly with our
   Versatile Data Pipeline, allowing for streamlined unstructured data ETL and
   model serving workflows.
2. No Code Console Builder: Easily utilize models defined with **⚗️ Instill
   Model** as modularized AI components via **📺 Instill Console**, facilitating
   the integration of models into downstream tasks without requiring any coding.
3. AutoML Feature (Coming Soon): With the upcoming AutoML feature, **⚗️ Instill
...
own custom model with **Instill Model** for local deployment on **🔮 Instill
Core**.

## Prerequisites

1. **Docker**: Both **🔮 Instill Core** and **⚗️ Instill Model** use **Docker**
...
2. **⌨️ Instill CLI**: The easiest way to launch **🔮 Instill Core** for local
   deployment is via the **⌨️ Instill CLI**. To install **⌨️ Instill CLI**
...
4. Now that **🔮 Instill Core** has been deployed, you can access **📺 Instill
...
For further details about launching **🔮 Instill Core**, we recommend that you
...
Please note that this guide assumes you have a basic understanding of machine
learning and can code in Python. If you are new to these concepts, we recommend
that you look at our [quick start
guide](/docs/latest/quickstart#authorisation?utm_source=tutorial&utm_medium=link&utm_campaign=model-oss-llm)
which introduces our no-code **💧 Instill VDP** pipeline builder, and also take
...
<ZoomableImg src="/tutorial-assets/model-serving-on-instill-core/T1.png" alt="2.
Configure Model Settings" />

You have now created an empty model namespace on **⚗️ Instill Model**. In the
next sections of this tutorial we will show you how to define your own custom
model for deployment!

## Step 2: Create a Model Config

To prepare a model to be served with **⚗️ Instill Model**, you first need to
...
Importantly, you must replace `{user_id}` with `admin`, and replace `{model_id}`
for `tinyllama` - the same Model ID that was specified in [Step
...
servable model. These are required to properly define a model class for **⚗️
Instill Model**.
...
[TinyLlama-1.1B-Chat](https://huggingface.co/TinyLlama/TinyLlama-1.1B-Chat-v1.0)
model falls under the `text_generation_chat` AI task, and so we will make use of
the `construct_text_generation_chat_metadata_response` helper function.
...
the trigger request from **⚗️ Instill Model**, contains the necessary logic to
...
define the input format for the text generation chat AI task, and the
`construct_text_generation_chat_infer_response` function from `instill.helpers`
is used to format the model output into the appropriate response format.
...
Awesome 😎, you have now defined your own custom model class for model serving
with **⚗️ Instill Model**. In the next step, we will show you how to build and
deploy this model locally on **🔮 Instill Core**!
...
To push the model image to the **🔮 Instill Core** instance we will need to be
able to login to Docker. To do this, you first need to create an API token by:
...
<ZoomableImg src="/tutorial-assets/model-serving-on-instill-core/T2.png" alt="2.
Create API Token" />
...
**⚗️ Instill Model** will then automatically allocate the resources required by
...
offline and **⚗️ Instill Model** is still in the process of allocating resources
and deploying it (this may take a few minutes). Once this status changes to
`Active`, your model is ready to serve requests. 🚀
...
Congratulations on successfully deploying and serving a custom model with **⚗️
Instill Model** and **🔮 Instill Core**! 🎉
...
3. Defined and implemented a custom model class for the TinyLlama-1.1B-Chat
   model.
4. Built and deployed your model image.
5. Tested your model's inference capabilities via the Console and API.

**⚗️ Instill Model** and **🔮 Instill Core** together provide a powerful,
streamlined solution for managing and deploying your own deep learning models.

Excitingly, you can now connect your own custom models via the **⚗️ Instill
Model** [**AI component**](../docs/component/ai/instill) to construct bespoke
**💧 Instill VDP** pipelines tailored to your unstructured data ETL
requirements. Please see the [Create a Pipeline](../docs/vdp/create) page for
more information on building **💧 Instill VDP** pipelines with **📺 Instill
Console**.
