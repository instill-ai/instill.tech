...
1. Please ensure that you have installed the latest version of the [Python
   SDK](/docs/latest/sdk/python-sdk) by running:
   ```bash
   pip install instill-sdk
   ```
2. **Docker**: Both **🔮 Instill Core** and **⚗️ Instill Model** use **Docker**
...
3. **⌨️ Instill CLI**: The easiest way to launch **🔮 Instill Core** for local
   ...
   ```
4. **Launch 🔮 Instill Core**: To launch simply run:
   ```bash
   inst local deploy
   ```
...
5. Now that **🔮 Instill Core** has been deployed, you can access **📺 Instill
...
fields, please refer to the [Create Namespace](/docs/model/create/namespace) page.
...
`instill.yaml`. Within the Python SDK, you can run the following helper command
...
Now we can login to the Docker register in the **🔮 Instill Core** instance by
running:
```bash
...
Again, you must remember replace `USER_ID` with `admin`, and replace `MODEL_ID` for
`tinyllama`.
...
down the local **🔮 Instill Core** instance to free up system resources. You can
do this using the **⌨️ Instill CLI** command:
...
6. Undeployed the local **🔮 Instill Core** instance.
...
Please see the [Create Pipeline](../docs/vdp/create) page for more information
