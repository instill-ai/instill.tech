...
2. import and deploy the **Metatron GPT-2 Medium model (345m)** model from a specific tag (compatible with GPUs on your machine) of GitHub repository [`instill-ai/model-gpt2-megatron-dvc`](https://github.com/instill-ai/model-gpt2-megatron-dvc) with ID **gpt2**,
...
The model only supports GPU deployment. By default, VDP can access all your GPUs. Assume that you have N GPUs, please pick the corresponding Git Tag named with fp32-345m-N-gpu to deploy.
...
<CtaButton
  ...
  link="/get-access/?utm_source=tutorial&utm_medium=link&utm_campaign=vdp-discord-text-generation?utm_source=blog&utm_medium=link&utm_campaign=vdp-discord-text-generation"
/>
