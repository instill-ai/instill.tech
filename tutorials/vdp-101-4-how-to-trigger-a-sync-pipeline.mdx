...
```bash cURL(url)
curl -X POST http://localhost:8080/v1alpha/pipelines/vdp-101-sync/trigger -d '{
  "task_inputs": [
    {
      "detection": {
...
```bash cURL(multipart)
curl -X POST http://localhost:8080/v1alpha/pipelines/vdp-101-sync/trigger-multipart \
--form 'file=@"dog.jpg"' \
--form 'file=@"horse.jpg"'
```
...
:::info{type=info}
For further details of the Python `requests.post()` method, we refer readers [here](https://www.w3schools.com/python/ref_requests_post.asp).
:::
