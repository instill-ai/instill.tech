---
title: "VDP 101 - 4. How to Trigger a SYNC Pipeline?"
lang: "en-US"
draft: false
...
In the previous tutorial [VDP 101 - 3. Create Your First Pipeline on VDP](tutorials/vdp-3-create-your-first-pipeline), you have learnt how to create a pipeline in `SYNC` mode using VDP Console. Now we are going to learn how to trigger this pipeline step-by-step. 

Depending on the use case, users can create a pipeline in `SYNC`, `ASYNC`, or `PULL` mode. The pipeline mode is determined by the combination of selected source and destination. Further details can be found in [VDP 101 - 6. Pipeline Modes - SYNC, ASYNC and PULL](/tutorials/vdp-101-6-pipeline-modes). For this tutorial, we use `SYNC` mode for showcasing. Let's not be bothered by the other two modes for just now.
...
Create and activate an Conda virtual environment named `vdp-streamlit` with Python 3.8:
...
:::info{type=info}
`requirements.txt` file can be found in all of our VDP examples under `/examples/`
:::
...
VDP currently support three input protocols to support `HTTP/gRPC` protocols. When triggering the pipeline with HTTP protocols (i.e. RESTful), users can send their data via **HTTP POST Requests** or **HTTP Multipart Requests**. You can find these examples in `/examples/vdp-101/sync/`
...
- **base64:** clients upload image filesin base64 format, encoding images into UTF-8 Strings. In this tutorial example, we convert JPG to BASE64 using **base64** library. An example can be found in `sync-http-base64.py`.
...
In addition to image objects, Versitile Data Pipeline (VDP) supports extensive data types that are commonly used in AI tasks. You can find further details in our [AI Tasks defined by VDP](/docs/core-concepts/ai-task), or the Python examples in `/examples/`. 

Okay, we have learnt how to tigger pipelines on VDP. Let's see how to parse pipeline responses in our next tutorial → [VDP 101 - 5. How to Parse Responses from SYNC Pipelines](/tutorials/vdo-101-5-hoe-to-parse-vdp-responses).
