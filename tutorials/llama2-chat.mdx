...
- **Docker and Docker Compose** - Instill Core uses Docker Compose (specifically, **`Compose V2`** and Compose specification) to run all services locally. Please install the latest stable **[Docker](https://docs.docker.com/get-docker/)** and **[Docker Compose](https://docs.docker.com/compose/install/)**.
...
Once all the services are up, the no-code Console is available at **[http://localhost:3000](http://localhost:3000/)**. Log in with username **admin** and the default password **password** (you can change this later).
...
```bash
{
  ...
  "taskOutputs": [
    {
      "textGenerationChat": {
        "text": "Oh no, I'm so sorry to hear that you're not feeling well today! 😔 Can you tell me more about how you're feeling? Sometimes talking about it can help. Is there anything in particular that's bothering you, or is it just a general feeling of not being well?"
      }
    }
  ]
}
...
<ZoomableImg
  ...
  alt="Where to get Instill Cloud API Token in Settings"
/>
...
**maxNewTokens: 1024** specifies the maximum number of tokens the model should generate in the response.
