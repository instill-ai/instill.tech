...
<InfoBlock type="info" title="info">
  Instill Model currently is in a transition state of adopting containerized model serving feature,
  which brings the benefit of isolated dependencies environment for each model. Stay tuned for more updates!
</InfoBlock>
...
We have currently deprecated `Hugging Face` and `ArtiVC` as model input source, but keeping `Github` and `Local` at the moment, we will fully move to `Container` format in a near future.
