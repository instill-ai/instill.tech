...
with comments and sample codes to describe what they do
```bash
instill init
```
3. Modify [model config](#model-configuration) file (`instill.yaml`) to
  describe your model's dependencies.
4. Modify `model.py` file which defines the model class that will be decorated
