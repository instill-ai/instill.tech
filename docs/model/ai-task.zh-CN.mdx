...
When you create a model on Instill Model, you need to specify which standardised AI tasks the model can be catagorized in.

In a data pipeline, a model is the core component designed to solve a specific AI task.
By standardising the data format of model outputs into AI tasks, models become modularized: you can freely switch to use different models in a pipeline as long as the model is designed for the same task.
This also means that the stream of data produced by models in VDP have a standard format for data integration or ETL pipelines.
...
The above tasks focus on analysing and understanding the content of data in the same way a human does.
The goal is to make a computer/device provide a description for the data that is as complete and accurate as possible.
...
Text Generation Chat is a Generative AI task to generate new text from text inputs in chat style.
Generally, the task takes a series of conversation as the input, and produces new response on the prompts.
The task can perform conversation and even answer question based on previous context.
...
The goal of VQA is to teach machines to understand the content of an image and answer questions about it in natural language.
...
- and wrap this information along with the raw model output in a _standard_ format.
...
If you'd like to have **support for a new task**, you can request it in the **#give-feedback** channel on [Discord](https://discord.gg/sevxWsqpGh).
