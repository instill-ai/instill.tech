...
To build a servable model image for **Instill Model**, follow the steps:
...
1. Next, docker login to `Instill Core` instance
...
2. Now we can push the model onto `Instill Core` instance
  - self-host `Instill Core`
```bash
instill_push -t {your-image-tag} localhost:8080
```
  - `Instill Cloud`
```bash
instill_push -t {your-image-tag} api.instill.tech
```
3. After seeing this message, it means the model image is successfully pushed
```bash
...
As the model image is pushed, **Instill Model** will automatically allocate necessary resources for your model and deploy it.
Depends on your model image size and the type of hardware you specified for the model, it could take awhile for the model to go online.
You can go to the `Versions` tab under your model namespace to see the state.
