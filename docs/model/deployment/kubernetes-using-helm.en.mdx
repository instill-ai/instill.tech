...
lang: "en-US"
draft: false
description: "Learn about how to deploy Instill Model https://github.com/instill-ai/model in Kubernetes using Helm"
---

Kubernetes offers an elastic way of managing Instill Model to persist the services and scale resources horizontally and vertically.

## Setup

Instill Model currently supports deploying on Kubernetes.
...
Check out the Instill Model Helm repository on [ArtifactHub](https://artifacthub.io/packages/helm/instill-ai/model).

To add the Instill Model Helm repository, simply run
...
<InfoBlock type="info" title="info">
  Instill Model and its Helm chart are still in a pre-release Alpha version, so
...
And visit the `api-gateway-base` ([http://localhost:7080](http://localhost:7080)) and Console ([http://localhost:3000](http://localhost:3000)) by port forwarding:

```bash
  export API_GATEWAY_BASE_POD_NAME=$(kubectl get pods --namespace instill-ai -l "app.kubernetes.io/component=api-gateway-base,app.kubernetes.io/instance=base" -o jsonpath="{.items[0].metadata.name}")
  export API_GATEWAY_BASE_CONTAINER_PORT=$(kubectl get pod --namespace instill-ai $API_GATEWAY_BASE_POD_NAME -o jsonpath="{.spec.containers[0].ports[0].containerPort}")
  ...
  kubectl --namespace instill-ai port-forward $API_GATEWAY_BASE_POD_NAME 7080:${API_GATEWAY_BASE_CONTAINER_PORT}
...
Then, install the Instill Model chart:

```bash
# Install the Instill Model chart
helm install model instill-ai/model --devel --namespace instill-ai --create-namespace
```

And visit the `api-gateway-model` ([http://localhost:9080](http://localhost:9080)) and Console ([http://localhost:3000](http://localhost:3000)) by port forwarding:

```bash
export API_GATEWAY_MODEL_POD_NAME=$(kubectl get pods --namespace instill-ai -l "app.kubernetes.io/component=api-gateway-model,app.kubernetes.io/instance=model" -o jsonpath="{.items[0].metadata.name}")
export API_GATEWAY_MODEL_CONTAINER_PORT=$(kubectl get pod --namespace instill-ai $API_GATEWAY_MODEL_POD_NAME -o jsonpath="{.spec.containers[0].ports[0].containerPort}")
kubectl --namespace instill-ai port-forward $API_GATEWAY_MODEL_POD_NAME 9080:${API_GATEWAY_MODEL_CONTAINER_PORT}
```
...
and clean up all Model-related persistent volumes:
