...
```bash
# for Instill Cloud
instill_push -t VERSION_TAG -u api.instill.tech

# for a local Instill Core instance
instill_push -t VERSION_TAG -u localhost:8080
```
...
`Versions` tab under your model namespace to see the current state.
