...
To prepare a model version to be served on **Instill Model**, you will have to follow the following steps locally on your PC:
...
- If the model requires GPU
- Python version
  - currently we only support python `3.11`
- Required list of system packages
  - the model image is based on ubuntu 20.04 LTS
- Required list of python packages
- your Instill Core namespace
...
After all the required model files are prepared, we can now [build](build) the custom model image with verion as tag.
