...
```shellscript deploy
curl -X POST http://localhost:8080/model/v1alpha/users/<user-id>/models/<model-id>/deploy
```

```shellscript undeploy
curl -X POST http://localhost:8080/model/v1alpha/users/<user-id>/models/<model-id>/undeploy
```
