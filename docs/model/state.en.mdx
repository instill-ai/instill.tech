...
A deployed model version can assume a number of different states. To understand
these states, we first need to introduce the concept of a replica.

At runtime, a model version deployment consists of a number of replicas. These
are essentially clones of the model class that are launched in separate
processes. The number of replicas is autoscaled according to the model traffic.
Please refer to the [Ray Serve](https://docs.ray.io/en/latest/serve/index.html)
documentation for further information regarding replicas.
...
<ZoomableImg
  src="/docs-assets/model/machine_state_diagram_v3.mp4"
