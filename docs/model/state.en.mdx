...
- When resources are still being initially allocated to deploy the model version, the state is `STARTING`.
...
- When there is `0` replica of this model version, the state is `OFFLINE`, and it will go through cold boot when the next request comes in.
