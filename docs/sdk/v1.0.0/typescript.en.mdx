...
- accessToken
  - local
    ...
    - password: `password`
  - production
    - go the setting page and create a access token. `https://console.instill.tech/settings`
...
const client = new InstillClient(
    ...
    "v1alpha", // appVersion
    "" // accessToken get it console API token from `https://console.instill.tech/settings`
);
```
...
export default function TypescriptSdkDemo() {
  ...
  const client = new InstillClient(
    ...
    "v1alpha",
    "" // get console API token from `https://console.instill.tech/settings`
  );
...
> Make sure you have vdp upruning in your local.

- To run the `VDP` locally check the repository for [VDP](https://github.com/instill-ai/vdp)

```typescript
import { useEffect, useState } from "react";
import InstillClient, { Nullable, User } from "@instill-ai/typescript-sdk";
