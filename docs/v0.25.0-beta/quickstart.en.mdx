...
Begin by navigating to the **Connectors** page on the left sidebar.
...
1. Click on **+ Add Connector**, add select **OpenAI** on the Modal.
...
1. Click on **+ Add Connector**, add select **Stability AI** on the Modal.
...
2. Click **Add Field +** and select **Text**. Fill in the **Title** with _Prompt_. The reference key for this field will be `prompt`. Click **Save**.
3. Click **Add Field +** and select **Text**, Fill in the **Title** with _Sticker Shape_. Similarly, the reference key for this field will be `sticker_shape`. Click **Save**.
...
5. Click the **Advanced configuration** link on the `openai_0` connector to open configuration right panel.
...
🎉 Congratulations! Your first VDP pipeline is all set up and ready to go. Now, simply click on the **Run** button located in the top right corner.
...
```
a yellow plane soaring through a clear blue sky, sticker style, flat icon, vector, die-cut round sticker with white border
```
...
1. Click on **Settings** on the left sidebar of the main page in the console.
