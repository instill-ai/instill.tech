...
- **VDP Connectors** (including [data source connectors](/docs/source-connectors/overview), [data destination connectors](/docs/destination-connectors/overview) and [model connectors](/docs/core-concepts/model#model-connector-coming-soon)): open-source and available under the MIT License
...
On the other hand, we've released [Instill Cloud](/docs/instill-cloud/getting-started), a fully-managed public cloud service built upon VDP that offers additional features.
With the license, we don't have to worry about our project being taken by some other companies for monetization.

### Why is VDP connectors open-source?

We want to build VDP as an open platform that integrates with other awesome tooling and services.
And we think the best way to build these integrations is via open source, no matter it is a data connector that connects data to a warehouse, application, or database, or a model connector that connects models from self-hosted or 3rd party platforms.
