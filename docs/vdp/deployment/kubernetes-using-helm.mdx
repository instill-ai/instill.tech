...
```bash
helm install vdp instill/vdp --devel --namespace instill-ai --create-namespace
```
...
```bash
export API_GATEWAY_VDP_POD_NAME=$(kubectl get pods --namespace vdp -l "app.kubernetes.io/component=api-gateway-vdp,app.kubernetes.io/instance=vdp" -o jsonpath="{.items[0].metadata.name}")
export API_GATEWAY_VDP_CONTAINER_PORT=$(kubectl get pod --namespace vdp $API_GATEWAY_VDP_POD_NAME -o jsonpath="{.spec.containers[0].ports[0].containerPort}")
kubectl --namespace vdp port-forward $API_GATEWAY_VDP_POD_NAME 8080:${API_GATEWAY_VDP_CONTAINER_PORT}
```
