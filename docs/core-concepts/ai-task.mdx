...
| 🔮 Model                                                                                                                                 | Sources                                                                  | Framework | CPU | GPU |
| ---------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------ | --------- | --- | --- |
| [Keypoint R-CNN R50-FPN](https://github.com/facebookresearch/detectron2/blob/main/configs/COCO-Keypoints/keypoint_rcnn_R_50_FPN_1x.yaml) | [GitHub-DVC](https://github.com/instill-ai/model-keypoint-detection-dvc) | PyTorch   | ✅  | ✅  |
...
| 🔮 Model                                                                         | Sources                                                                                                                                                      | Framework | CPU | GPU |
| -------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------- | --- | --- |
| [Stable Diffusion v2.1](https://huggingface.co/stabilityai/stable-diffusion-2-1) | [GitHub-DVC](https://github.com/instill-ai/model-diffusion-dvc), [Local](https://artifacts.instill.tech/vdp/sample-models/stable-diffusion-2-1-fp32-cpu.zip) | ONNX      | ✅  | ✅  |

:::info{type=tip}

Importing [Stable Diffusion v2.1](https://github.com/instill-ai/model-diffusion-dvc) from GitHub will take a while. Alternatively, you can download the model locally as a one-time effort.

**Step 1**: Download **Stable Diffusion v2.1 CPU** sample model.

```bash
curl https://artifacts.instill.tech/vdp/sample-models/stable-diffusion-2-1-fp32-cpu.zip --output stable-diffusion-2-1-fp32-cpu.zip
```
...
:::info{type=tip}
Depending on your Internet speed, importing [Megatron-GPT2-345m](https://github.com/instill-ai/model-gpt2-megatron-dvc) from GitHub will take a while.

The model only supports GPU deployment. By default, VDP can access all your GPUs.
Assume that you have **N** GPUs, please pick the corresponding model instance tagged with `fp32-345m-N-gpus` to deploy.
