...
- **Text to Image** - generates images from input text prompts
- **Text Generation** - generates texts from input text prompts
- The list is growing ... 🌱
...
properties:
  ...
  text_generation:
    description: "Generate natural language from input data"
...
```json
{
  "text_to_image": {
    "images": [
      ...
      "oADAMBAAIRAxEAPwD2p76rBDHU2KHMpuE"
    ]
  }
}
```
...
Text Generation is a Generative AI task to predicts the next word given input words.
Generally it can be used in two major ways when receives text input:
* Given an incomplete sentence, complete it.
* Continue a story given the first sentence.
...
```json
{
    ...
    "task_outputs": [
        {
            "text_generation": {
                "text": "The winds of change are blowing strong..."
            }
        }
    ]
}
