...
<InfoBlock type="info" title="info">
  Try our [Import GitHub models](./import-models/github) guideline to import a
  model from GitHub
</InfoBlock>
...
and the raw model output will be wrapped in a [standard format](/ai-task#unspecified-ai-task).
...
🙌 After preparing your model to be Instill Model compatible, check out [Import Models](/docs/latest/import-models/overview) to learn about how to import the model into Instill Model from different sources.
...
You can prepare the post-processing model the same way as the [pre-processing model](#pre-processing).
...
Check the [standardised output for Unspecified AI task](ai-task#unspecified-ai-task).
