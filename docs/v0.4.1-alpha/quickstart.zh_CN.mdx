...
In this guide, we'll walk you through building your first VDP pipeline, focusing on a basic Generative pipeline using the [Stability AI AI Connector](/docs/latest/vdp/ai-connectors/stability-ai), which seamlessly integrates with Stability AI's services.
...
For more information on pipeline states, refer to the [documentation](/docs/latest/core-concepts/pipeline#state).
...
Each model in the Model Hub is designed to handle a specific [AI task](/docs/latest/core-concepts/ai-task).

For detailed information about a specific model, click on its ID and review the Description and Setting sections.
If you want to import the model into VDP, refer to the VDP documentation on the [Instill Model AI connector](/docs/latest/vdp/ai-connectors/instill-model).
