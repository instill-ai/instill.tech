...
specifically focusing on a generative pipeline using the [OpenAI](./vdp/connectors/openai) and [Stability AI](./vdp/connectors/stability-ai) Connectors to generate digital stickers.
...
4. Optionally, specifiy which organization is used for the request.
5. Click **Save**.
...
🎉 Congratulations! You have successfully set up your first VDP pipeline.
Now, click **Test** in the top right corner.

You can now test the pipeline with real data. For example, let's geneate a sticker with a yellow plan on the sky.
...
You can see that the OpenAI ChatGPT model has enriched our prompt into:
...
If you want to import the model into VDP, refer to the VDP documentation on the [Instill Model AI connector](./vdp/connectors/instill-model).
