...
A **Component** is a fundamental building block used to construct a pipeline in VDP. VDP has two types of components: **Connectors** and **Operators**. Connector and Operator are served by the [`pipeline-backend`](https://github.com/instill-ai/pipeline-backend).
