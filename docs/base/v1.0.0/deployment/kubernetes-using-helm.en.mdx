...
lang: "en-US"
draft: false
description: "Learn about how to deploy Instill Base https://github.com/instill-ai/base in Kubernetes using Helm"
---

Kubernetes offers an elastic way of managing Instill Base to persist the services and scale resources horizontally and vertically.

## Setup

Instill Base currently supports deploying on Kubernetes.
...
Check out the Base Helm repository on [ArtifactHub](https://artifacthub.io/packages/helm/instill-ai/base).

To add the Base Helm repository, simply run
...
<InfoBlock type="info" title="info">
  Instill Base and its Helm chart are still in a pre-release Alpha version, so
  the `--devel` flag is necessary for the access of Helm charts.
</InfoBlock>

## Install

Install the Instill Base chart:
...
```bash
  ...
  kubectl --namespace instill-ai port-forward $API_GATEWAY_BASE_POD_NAME 8080:${API_GATEWAY_BASE_CONTAINER_PORT}
...
and clean up all Base-related persistent volumes:
